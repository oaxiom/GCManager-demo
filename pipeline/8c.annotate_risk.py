...
##### Get the key snps
key_snps = []
oh = open(os.path.expanduser('~/gcm/static_data/Risk/key_snps.txt'), 'rt')
for line in oh:
...
output = open(f'{PID}.risk.txt', 'wt')
output.write(f'chrom\trsid\tgenotype\n')
for r in results:
    # Format the results useful for the next stage
    # GATK reports the het/hom, although it's not
    # required for GWAS, which disregards het/hom
    # Blasted microarrays...
    chrom = r[0]
    pos = r[1]
    rsid = r[2]
...
print('Stage 8: Finished Risk GWAS annotation')
